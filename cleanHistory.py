...
def cleaner(pList):
    ...
    for sid, series in enumerate(pList):
        if sid == 0:
            ...
            dfOut = pd.DataFrame(np.array(dfOut.merge(dfN, on='DateTime', how='outer'),),columns=labels)

    # All of the stocks have been added to the dataframe, now clean it up
    dfClean = dfOut.dropna()
    # dfClean.index = dfClean['DateTime']
    dfClean['DateTime']=pd.to_datetime(dfClean['DateTime'])
    dfClean = dfClean.sort(columns='DateTime',ascending=True)
